...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    public Invoice findByNumberInvoice(String numberInvoice) {
        ...
        Customer customer =
                modelMapper.map(customerClient.findByID(invoice.getCustomerId()).getData(), Customer.class);
        invoice.setCustomer(customer);
        List<InvoiceItem> items = invoice.getItems().stream()
                .map(invoiceItem -> {
                    Product product =
                            modelMapper.map(productClient.findById(invoiceItem.getProductId())
                                    .getData(), Product.class);
